...
<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"
  ...
  <spdx:SpdxDocument rdf:about="https://example.com/test">
    ...
    fn test_parse_cyclonedx_mlbom_fixture() {
        ...
        assert_eq!(
            ml_info.training_datasets[0].name.as_deref(),
            Some("wikipedia-2.5B")
        );
        assert_eq!(
            ml_info.training_datasets[0].purl.as_deref(),
            Some("pkg:generic/wikipedia/2.5B")
        );
        assert_eq!(
        ...
        assert_eq!(
            ml_info.training_datasets[1].purl.as_deref(),
            Some("pkg:generic/bookscorpus/800M")
        );
        assert_eq!(
