...
#[test]
fn privileged_pull_request_target_is_not_classified_as_an_ordinary_pr() {
    for reference in ["refs/pull/42/merge", "42/merge", "refs/heads/main"] {
        let metadata = hosted("pull_request_target", reference);
        assert!(derive_trust_context(Some(&metadata), false).is_err());
    }
}

#[test]
fn full_pull_request_refs_require_a_numeric_merge_identity() {
    for reference in [
        ...
        assert!(derive_trust_context(Some(&hosted("pull_request", reference)), false).is_err());
    }
}
