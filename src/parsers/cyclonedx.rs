...
impl CycloneDxParser {
    ...
    fn convert_component(&self, cdx: &CdxComponent) -> Component {
        ...
        if let Some(data_component) = &cdx.data_component {
            ...
            dataset_info.sensitivity_classifications = data_component.sensitivity_data.clone();

            if let Some(governance) = &data_component.governance {
                // governance.owners is Vec<String>, clone directly
                dataset_info.governance_owners = governance.owners.clone();
            }
